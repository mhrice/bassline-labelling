...
def run_crepe(audio_path, step_size=10):
    sr, audio = wavfile.read(str(audio_path))
    time, frequency, confidence, activation = crepe.predict(audio, sr, viterbi=True, step_size=step_size)
...
def load_audio(audio_path, cached_amp_envelope_path, default_sample_rate, detect_amplitude, save_amp_envelope, step_size=0.01):
    if cached_amp_envelope_path.exists():
        ...
        filtered_amp_envelope = filtfilt(b, a, scaled_amp_envelope)[::(sr // (int(1 / step_size)))]
...
def process(freqs,
    ...
    for x_s in notes:
        ...
        time_start = step_size * seg_start
        time_end = step_size * seg_end
        ...
        for n in output_notes:
            ...
            if np.any(onsets[n_s:n_f] > 0.7):
                ...
                for idx in onset_idxs_within_note:
                    if idx[0] > last_onset + int(min_duration / step_size):
                        new_note = n.copy()
    ...
    if detect_amplitude:
        ...
        for n in output_notes:
            ...
            if f - s > (min_duration / step_size):
                # TODO: make noise floor configurable
                noise_floor = step_size  # this will vary depending on the signal
                ...
                timed_note['start'] = s_adj * step_size
                timed_note['finish'] = f_adj * step_size
                timed_output_notes.append(timed_note)
            else:
                timed_note['start'] = s * step_size
                timed_note['finish'] = f * step_size
    ...
    return timed_output_notes, f"{output_filename}.{output_label}.mid"
